...
例子：样本空间$\Omega=\{0,1,2,\cdots,n,\cdots\}$
...
(2)结合律\\(A\cup B)\cup C=A\cup (B\cup C)交集同理\\
(3)分配律\\(A\cup B)\cap C=(A\cap C)\cup (B\cap C)\\
(A\cap B)\cup C=(A\cup C)\cap (B\cup C)
(4)对偶性\\ \overline{A\cup B}=\overline{A}\cap \overline{B}\\
...
若A、B独立，则$A\与\overline{B},A\与\overline{B,}B\与\overline{A},\overline{A}\与\overline{B}$独立
...
(3)$P(\sum^\infin_{i=1}A_i|B)=\sum^\infin_{i=1}P(A_i|B)$
...
$$
\int^x_0 \lambda e^{-\lambda t}\mathrm{d}t\\
\begin{aligned}
...
假设题目给出$Y=ax+b$，并且通过密度函数变换得出$Y=\frac{x-\mu}{\sigma}$，可以得知$\frac{1}{\sigma}=a$
...
用一个帽型表示，分布函数:$F(x,y)=P\{X\le x,Y\le y\}$联合分布
...
(5)$x_1<x_2,y_1<y_2,P\{x_1<Y<x_2,y_1<Y<y_2\}=F(x_2,y_2)-F(x_2,u_1)-F(x_1,y_2)+F(x_1,y_1)$
...
\end{cases}\\
G:x^2+y^2\le r^2\\
\int^{+\infin}_{-\infin}\int^{+\infin}_{-\infin}f(x,y)\mathrm{d}x\mathrm{d}y\\=\iint_G C \mathrm{d}x\mathrm{d}y=C\pi r^2=1\\
C=\frac{1}{\pi r^2}
$$
...
表达方式:$f_X(x)=\int^{+\infin}_{-\infin}f(x,y)\mathrm{d}y=F'(x,+\infin)$
...
$$
F_Z(\xi)=P\{Z\le \xi\}=P\{g(X,Y)\le\xi\}\\=\iint_{D_\xi}f(x,y)\mathrm{d}x\mathrm{d}y\\
...
复合情况EY=\int g(x)f(x)\mathrm{d}x\\
二维情况EZ=\sum_i\sum_jg(x_i,y_j)P_{ij}\\
Z=x^2-Y,EZ=(x_1^2-y_1)*P_{11}+(x_1^2-y_2)*P_{12}+\cdots
$$
...
凑-\lambda，然后积分是无限减到0的，当为0时是1，所以最后是-1\\
...
\begin{aligned}
分布&|&定义&|&EX&|&DX\\
0-1&|&P(X=k)=p^k(1-p)^{1-k},k=0,1&|&EXp&|&DX\\
...
> 以下为姜老师所讲内容，考虑到弹幕所言可能不靠仅进行粗略记录，待后续补全
